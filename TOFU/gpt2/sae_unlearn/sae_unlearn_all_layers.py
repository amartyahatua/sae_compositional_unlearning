...
class Config:
    ...
    # ── SAE sweep ──────────────────────────────────────────────────────────────
    DICT_SIZES   = [65536]
    # LAYERS       = list(range(0,12))        # layer 10 for small, 23 for medium
...
def select_features(forget_feats, retain_feats, top_k, threshold):
    ...
    Returns:
        top_idx      : 1D LongTensor of selected feature indices (CPU)
        top_contrast : 1D FloatTensor of contrast scores for those features (CPU)
    """
    ...
    top_contrast_vals, top_idx = torch.topk(contrast, k=k)

    print(f"      Features selected : {k}")
    print(f"      Contrast  min/mean/max : "
          f"{top_contrast_vals.min():.4f} / "
          f"{top_contrast_vals.mean():.4f} / "
          f"{top_contrast_vals.max():.4f}")

    return top_idx.cpu(), top_contrast_vals.cpu()
...
def _patch_hidden(h, sae, target_features, device,
                  retain_means=None, suppression_weights=None):
    """
    Core patching logic shared by all hooks.

    h                   : (B, T, D) hidden state tensor
    target_features     : 1D LongTensor of feature indices (CPU)
    retain_means        : (dict_size,) tensor — mean ablation if provided
    suppression_weights : (K,) tensor — per-feature scale in [0,1]
                          if provided, multiply each feature by its weight
                          (used for exponential suppression mode)

    Priority: suppression_weights > retain_means > zero ablation
    """
    orig_shape = h.shape
    ...
    with torch.no_grad():
        ...
        idx   = target_features.to(device)     # (K,)

        if suppression_weights is not None:
            # Exponential suppression: multiply each feature by its weight
            # weights shape (K,) → broadcast over (B*T, K)
            w = suppression_weights.to(device)  # (K,)
            feats[:, idx] = feats[:, idx] * w.unsqueeze(0)

        elif retain_means is not None:
            # Mean ablation
            feats[:, idx] = retain_means[idx].unsqueeze(0)
...
def _apply_hook(out, sae, target_features, device,
                retain_means=None, suppression_weights=None):
    """Unwrap GPT-2 block output tuple, patch hidden state, rewrap."""
    if isinstance(out, tuple):
        h, rest = out[0], out[1:]
        patched = _patch_hidden(h, sae, target_features, device,
                                retain_means=retain_means,
...
def make_zero_hook(sae, target_features, device):
    """Set selected SAE features to 0."""
    def hook(module, inp, out):
        return _apply_hook(out, sae, target_features, device)
    return hook


def make_mean_hook(sae, target_features, retain_feature_means, device):
    """Replace selected SAE features with retain-set mean activations."""
    means = retain_feature_means.to(device)
    def hook(module, inp, out):
        return _apply_hook(out, sae, target_features, device,
                           retain_means=means)
    return hook
...
def make_exp_suppression_hook(sae, target_features, contrast_scores,
                               device, alpha=1.0):
    """
    Exponential suppression hook.

    Each selected feature i is multiplied by:
        weight(i) = exp(-alpha * contrast_score(i))

    High contrast → weight near 0 → strong suppression
    Low contrast  → weight near 1 → gentle suppression

    alpha controls the steepness:
        alpha=0.5  — gentle curve
        alpha=1.0  — moderate (default)
        alpha=2.0  — aggressive, high-contrast features nearly zeroed
        alpha=5.0  — almost binary: high-contrast → 0, low-contrast → 1

    contrast_scores: (K,) FloatTensor, same order as target_features
    """
    # Normalise contrast scores to [0, 1] before applying exp
    # so alpha has consistent meaning regardless of contrast scale
    c = contrast_scores.float()
    if c.max() > c.min():
        c_norm = (c - c.min()) / (c.max() - c.min())   # (K,) in [0,1]
    else:
        c_norm = torch.zeros_like(c)

    weights = torch.exp(-alpha * c_norm)                # (K,) in (0, 1]

    print(f"      Exp suppression weights  min/mean/max : "
          f"{weights.min():.4f} / {weights.mean():.4f} / {weights.max():.4f}  "
          f"(alpha={alpha})")

    def hook(module, inp, out):
        return _apply_hook(out, sae, target_features, device,
                           suppression_weights=weights)
    return hook
...
def run_sae_unlearning(model, sae, forget_loader, retain_loader,
                       layer_idx, device, top_k, threshold,
                       ablation_mode, exp_alpha=1.0):
    """
    Full SAE-guided unlearning pipeline.

    ablation_mode: 'zero' | 'mean' | 'exp'
      zero : set selected features to 0
      mean : replace with retain-set mean
      exp  : multiply each feature by exp(-alpha * normalised_contrast)
             controlled by exp_alpha hyperparameter
    """
    ...
    target_features, contrast_scores = select_features(
        forget_feats, retain_feats, top_k, threshold
    )
    ...
    if ablation_mode == 'zero':
        ...
        retain_means = retain_feats.mean(dim=0)
        hook_fn = make_mean_hook(sae, target_features, retain_means, device)

    elif ablation_mode == 'exp':
        hook_fn = make_exp_suppression_hook(
            sae, target_features, contrast_scores, device, alpha=exp_alpha
        )
    else:
...
def run_author(author_name, author_indices, group,
               ...
               top_k, threshold, ablation_mode, exp_alpha, device):
...
def run_config(dict_size, layer_idx, model, tokenizer, author_to_samples,
               top_k, threshold, ablation_mode, exp_alpha, date_str):

    print(f"\n{'=' * 76}")
    print(f"  CONFIG  dict={dict_size}  layer={layer_idx}  "
          f"mode={ablation_mode}  top_k={top_k}  "
          f"threshold={threshold}  exp_alpha={exp_alpha}")
    print(f"{'=' * 76}")

    out_dir = (Path(Config.RESULTS_BASE_DIR)
               / f"dict_{dict_size}"
               / f"layer_{layer_idx:02d}"
               / f"{ablation_mode}_alpha{exp_alpha}")
    ...
    for author_name, group in all_authors:
        ...
        row = run_author(
            ...
            model, sae, tokenizer, layer_idx, dict_size,
            top_k, threshold, ablation_mode, exp_alpha, Config.DEVICE
        )
        if row:
            results.append(row)
...
def main():
    ...
    # curve
    # alpha = 1.0  — moderate(default)
    # alpha = 2.0  — aggressive, high - contrast
    # features
    # nearly
    # zeroed
    # alpha = 5.0  — almost
    # binary: high - contrast → 0, low - contrast → 1
    ...
    for dict_size in Config.DICT_SIZES:
        for layer_idx in Config.LAYERS:
            ...
            df = run_config(
                dict_size, layer_idx, model, tokenizer, author_to_samples,
                Config.TOP_K_FEATURES, Config.CONTRAST_THRESHOLD,
                Config.ABLATION_MODE, exp_alpha, date_str  # ← add exp_alpha
            )
